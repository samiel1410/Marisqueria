...
function BrandModal({ open, onClose, editing, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await api.post('/brands/update', { id: editing.id, name: name.trim() });
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div className="flex justify-between items-center p-6 border-b border-primary-100">
          <h3 className="text-lg font-bold">{editing ? 'Editar Marca' : 'Nueva Marca'}</h3>
          <button onClick={onClose} className="text-primary-400 hover:text-primary-700 text-xl leading-none">&times;</button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <Input label="Nombre *" required value={name} onChange={e => setName(e.target.value)} placeholder="Ej: Coca-Cola, Del Monte..." />
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto">
      <PageHeader
        ...
        action={<Button variant="brand" icon={Plus} onClick={openCreate}>Nueva Marca</Button>}
      />

      <div className="mb-6">
        <Input placeholder="Buscar marca..." value={search} onChange={e => setSearch(e.target.value)} className="max-w-sm" />
      </div>
      ...
      <DataTable
        ...
        empty={{ title: 'Sin marcas', description: 'Crea tu primera marca.', icon: Award }}
      />
